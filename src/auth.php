...
function change_password(string $currentPassword, string $newPassword, string $confirmPassword): void {
    ...
    if (strlen($newPassword) < 10 || !preg_match('/[A-Z]/',$newPassword) || !preg_match('/[a-z]/',$newPassword) || !preg_match('/\d/',$newPassword)) throw new RuntimeException('Use at least 10 characters with upper-case, lower-case and a number.');
    $email=(string)(user()['email'] ?? '');
    if ($email === '' || $currentPassword === '') throw new RuntimeException('Enter your current password.');
    // This verifies the current password with Supabase without exposing it or storing it locally.
...
function require_role(string $role): void {
    ...
    if (current_role() !== $role) {
        ...
        $home=current_role() === 'admin' ? 'admin_dashboard' : (current_role() === 'doctor' ? 'doctor_dashboard' : 'dashboard');
        redirect('?page=' . $home);
    }
}
