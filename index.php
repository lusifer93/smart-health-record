...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($form === 'register') { sign_up(trim($_POST['email']), $_POST['password'], trim($_POST['name'])); if (auth_token()) { flash('success', 'Welcome to CareNest â€” your private health space is ready.'); redirect('?page=dashboard'); } flash('success', 'Account created. Please sign in to continue.'); redirect('?page=login'); }
...
if ($form === 'medicine_delete') {
    ...
    foreach ($items as $item) {
        $sb->db('POST', 'medicines', '', [[
            ...
            'notes' => 'Extracted from prescription â€” verify with your clinician.',
        ]]);
    }
...
function gemini_generate(array $parts): string {
    ...
    if ($raw === false || $code >= 400) {
        $message = $json['error']['message'] ?? $error ?: 'Unknown error';
        throw new RuntimeException('Gemini AI request failed: ' . $message);
    }
...
<div class="page-heading"><h1>Appointments</h1><p>Request a time with a verified clinician. Your request is sent directly to their workspace.</p></div><div class="card p-4 mb-4"><h2 class="h5 mb-3">Available doctors</h2><?php if(!$doctors): ?><div class="empty-state"><i class="bi bi-person-check"></i>No verified doctors are available yet.</div><?php else: ?><div class="row g-3"><?php foreach($doctors as $doctor): ?><div class="col-md-6"><div class="border rounded-4 p-3 h-100"><div class="d-flex justify-content-between"><div><strong><?=h($doctor['full_name'])?></strong><div class="small text-muted"><?=h($doctor['specialization'] ?? 'Clinician')?> Â· <?=h($doctor['clinic_name'] ?? '')?></div><div class="small text-muted mt-1"><?=h($doctor['qualification'] ?? '')?></div></div><span class="role-chip">âœ“ Verified</span></div><form method="post" class="row g-2 mt-2"><input type="hidden" name="form" value="appointment_request"><input type="hidden" name="return_page" value="appointments"><input type="hidden" name="doctor_id" value="<?=h($doctor['id'])?>"><div class="col-md-6"><input class="form-control form-control-sm" type="datetime-local" name="preferred_at" required></div><div class="col-md-6"><input class="form-control form-control-sm" name="reason" placeholder="Reason (optional)"></div><div><button class="btn btn-sm btn-primary">Request appointment</button></div></form></div></div><?php endforeach ?></div><?php endif ?></div><h2 class="h5 mb-3">My appointment requests</h2><?php if(!$items): ?><div class="empty-state card"><i class="bi bi-calendar2-week"></i>No appointment requests yet.</div><?php endif; foreach($items as $i): ?><div class="card p-3 mb-2"><div class="d-flex justify-content-between"><strong><?=date('d M Y, H:i',strtotime($i['scheduled_at']))?></strong><span class="role-chip"><?=h($i['status'] ?? 'pending')?></span></div><div class="small text-muted mt-1"><?=h($i['reason'] ?? 'No reason provided')?><?=!empty($i['response_note'])?' Â· '.h($i['response_note']):''?></div></div><?php endforeach; }
